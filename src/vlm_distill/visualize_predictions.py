...
def _extract_payload(value: Any) -> tuple[dict[str, Any], str]:
    ...
    try:
        ...
        if isinstance(decoded, dict):
            return decoded, str(
                decoded.get("coordinate_system") or COORDINATE_SYSTEM_NORMALIZED_0_1000
            )
    except ValueError:
        pass
    ...
    if parsed.get("usable"):
        return {
            ...
            or COORDINATE_SYSTEM_NORMALIZED_0_1000,
        }, COORDINATE_SYSTEM_NORMALIZED_0_1000
        ...
        for line_number, line in enumerate(handle, 1):
            ...
            try:
                ...
                if coordinate_system not in (
                    ...
                    raise ValueError(
                        f"unsupported coordinate_system={coordinate_system!r}; expected normalized_0_1000"
                    )
                name = _output_name(sample_id, source, used)
                output_path = output_dir / name
                if source == output_path.resolve():
                    raise ValueError(
                        "refusing to write because source and output resolve to the same path"
                    )
                if output_path.exists() and not overwrite:
                    raise FileExistsError(f"output exists (use --overwrite): {output_path}")
                sidecar = output_path.with_suffix(".json")
                if write_sidecar and sidecar.exists() and not overwrite:
                    raise FileExistsError(f"sidecar exists (use --overwrite): {sidecar}")
                with Image.open(source) as image:
                    canvas = image.convert("RGB").copy()
                width, height = canvas.size
                draw = ImageDraw.Draw(canvas)
                font = _font(max(1, font_size))
                metadata = []
                for index, element in enumerate(elements):
                    summary["elements_total"] += 1
                    raw_bbox = element.get("bbox_norm", element.get("bbox"))
                    parsed_bbox = _bbox_value(raw_bbox)
                    item = {
                        "index": index,
                    ...
                    if parsed_bbox is None:
                        item["skip_reason"] = "missing or malformed bbox"
                        summary["elements_skipped"] += 1
                        metadata.append(item)
                        continue
                    try:
                        pixel_bbox = normalized_bbox_to_pixel(parsed_bbox, width, height)
                    ...
                    if pixel_bbox[0] == pixel_bbox[2] or pixel_bbox[1] == pixel_bbox[3]:
                        item["skip_reason"] = "bbox has zero pixel area"
                        summary["elements_skipped"] += 1
                        metadata.append(item)
                        continue
                    item["pixel_bbox"] = pixel_bbox
                    item["drawn"] = True
                    metadata.append(item)
                    color = PALETTE[index % len(PALETTE)]
                    draw.rectangle(pixel_bbox, outline=color, width=max(1, line_width))
                    label = _safe_label(
                        index,
                        element,
                        show_text=show_text,
                    ...
                    _draw_label(
                        draw,
                        (pixel_bbox[0], pixel_bbox[1] - font_size - 10),
                        label,
                        color,
                        font,
                        width,
                        ...
                        json.dumps(
                            {
                                ...
                                "image_width": width,
                                "image_height": height,
                                "coordinate_space": COORDINATE_SYSTEM_NORMALIZED_0_1000,
                                "element_count": len(elements),
                                "drawn_element_count": sum(1 for item in metadata if item["drawn"]),
                                "skipped_element_count": sum(
                                    1 for item in metadata if not item["drawn"]
                                ),
                                "elements": metadata,
                            },
                            ensure_ascii=False,
...
def run_from_config(config_path: Path, output_dir: Path, **kwargs: Any) -> dict[str, int | str]:
    ...
    return run_visualization(
        resolve_prediction_path(config.data),
        output_dir,
