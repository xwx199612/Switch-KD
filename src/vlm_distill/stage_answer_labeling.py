...
class HuggingFaceTeacher:
    ...
    def answer(self, sample: VlmSample) -> dict:
        ...
        answer = self._generate(image=image, prompt=prompt, sample=sample)
        answer = _normalize_teacher_answer(sample, answer)

        if sample.task == "screen_parsing" and _screen_parsing_quality_score(answer) <= 2:
            retry_prompt = _build_screen_parsing_retry_prompt(sample)
            retry_answer = self._generate(
                image=image,
                prompt=retry_prompt,
                sample=sample,
            ...
            if _screen_parsing_quality_score(retry_answer) >= _screen_parsing_quality_score(answer):
                answer = retry_answer
    ...
    def _generate(
        ...
        *,
        ...
        if sample.task == "screen_parsing" and _looks_degenerate_screen_output(answer):
            return ""
        return answer
...
def create_distillation_dataset(config: PipelineConfig, samples: list[VlmSample]) -> Path:
    output_path = config.data.distill_path
...
def _resize_teacher_image(image, resize_mode: str):
    ...
    target_height = 720 if mode == "720p" else 480
...
def _normalize_teacher_answer(sample: VlmSample, teacher_answer: str) -> str:
    if sample.task != "screen_parsing":
        return teacher_answer.strip()
    payload = _normalize_screen_parsing_payload(teacher_answer)
    return json.dumps(payload, ensure_ascii=False)


def _normalize_screen_parsing_payload(teacher_answer: str) -> dict[str, object]:
    payload = _empty_screen_parsing_payload()
...
def _empty_screen_parsing_payload() -> dict[str, object]:
    return {
        "elements": [],
    }
...
def _screen_parsing_quality_score(answer: str) -> int:
    payload = _normalize_screen_parsing_payload(answer)
...
def _build_screen_parsing_retry_prompt(sample: VlmSample) -> str:
