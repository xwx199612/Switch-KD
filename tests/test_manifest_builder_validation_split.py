...
def _config(tmp_path: Path):
    ...
    return SimpleNamespace(
        ...
        data=SimpleNamespace(
            ...
            image_dir=None,
            training_manifest_path=tmp_path / "raw.jsonl",
...
def test_labeled_split_preserves_original_images_and_cleans_source_image(tmp_path: Path):
    ...
    split_labeled_dataset(
        full_label_path=full,
        training_label_path=train,
        validation_label_path=validation,
        ratio=0.2,
        seed=42,
    )
...
def test_split_overwrite_and_rollback_preserve_existing_outputs(
    ...
    monkeypatch.setattr(
        "vlm_distill.labeled_split._atomic_jsonl",
