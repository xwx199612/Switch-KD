...
from vlm_distill.visualize_predictions import normalized_bbox_to_pixel, run_from_config, run_visualization
...
def test_run_from_config_uses_config_max_samples_when_cli_value_is_none(monkeypatch) -> None:
    ...
    monkeypatch.setattr("vlm_distill.visualize_predictions.load_config", lambda _path: _mock_config(25))
    monkeypatch.setattr("vlm_distill.visualize_predictions.resolve_prediction_path", lambda _data: Path("predictions.jsonl"))
...
def test_run_from_config_passes_none_when_config_and_cli_max_samples_are_none(monkeypatch) -> None:
    visualization = Mock(return_value={"images_written": 0})
    monkeypatch.setattr("vlm_distill.visualize_predictions.load_config", lambda _path: _mock_config(None))
    monkeypatch.setattr("vlm_distill.visualize_predictions.resolve_prediction_path", lambda _data: Path("predictions.jsonl"))
    monkeypatch.setattr("vlm_distill.visualize_predictions.run_visualization", visualization)
