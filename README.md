...
```text
A0 = Attention LoRA only
A1 = Attention LoRA + Full Projector
A2 = Attention LoRA + Projector LoRA
A3 = Attention LoRA + MLP LoRA + Full Projector
```

A3 tests whether adding the language-model MLP nonlinear feature transformation to distillation,
in addition to attention routing, improves Android TV full-screen parsing, text recognition, and
bbox grounding. Its high-fidelity deployment is a 4-bit NF4 LM base plus floating attention and
MLP LoRA plus a BF16 full-trained projector; the adapter remains active and is never merged.

Package and use the A3 deployment config (the A0/A1/A2 configs remain available):

```bash
vlm-distill package-adapter --config configs/lora_ablation/deploy/stage1_a3_4bit_base_bf16_adapter.yaml
vlm-distill predict --config configs/lora_ablation/deploy/stage1_a3_4bit_base_bf16_adapter.yaml
vlm-distill evaluate-predictions --config configs/lora_ablation/deploy/stage1_a3_4bit_base_bf16_adapter.yaml
```
...
`label` is the unified teacher-labeling flow. By default
labeling writes only `serialized parsing target`, `cached answer token ids`, and
...
Use `vlm-distill label --config ...` for both initial generation and resume.
Add `--overwrite` to regenerate every label and rebuild the labeled split.
...
student:
  ...
  output_dir: outputs/switch-kd/parsing_switch_kd_{response_profile}
  adapter_dir: outputs/switch-kd/parsing_switch_kd_{response_profile}/adapter
  merged_model_path: outputs/switch-kd/parsing_switch_kd_{response_profile}/merged_model
...
Parallel label helper:

```bash
bash scripts/run_parallel_switch_kd_precompute_4gpu.sh --clean-outputs label
```

This helper splits `outputs/switch-kd/parsing_manifest.jsonl` into four shard manifests, generates temporary configs in `configs/generated/`, launches four `label` workers with `CUDA_VISIBLE_DEVICES=0..3`, then merges shard teacher outputs back into `outputs/switch-kd/parsing_teacher_labels_480p_8bit.jsonl` after all workers succeed.

## Step 8

Batch test a merged student model.

Instead of using `infer_merged.py` one sample at a time, you can run the merged model on the whole manifest with `label`.
