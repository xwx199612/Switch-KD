...
EXPERIMENTS = [
    "stage1_a0_r16_attn",
    "stage1_a0_r32_attn",
    "stage1_a1_r16_attn_projector",
    "stage1_a1_r32_attn_projector",
    "stage1_a2_r16_attn_projector_lora",
    "stage1_a2_r32_attn_projector_lora",
    "stage1_a3_r16_attn_mlp",
    "stage1_a3_r32_attn_mlp",
    "stage1_a4_r16_attn_mlp_projector",
    "stage1_a4_r32_attn_mlp_projector",
]
    ...
    with prediction.open("r", encoding="utf-8-sig") as handle:
        for line_no, line in enumerate(handle, 1):
            ...
            try:
                ...
                result["focused_elements"] += sum(
                    1 for e in elements if isinstance(e, dict) and e.get("focused") is True
                )
                if row.get("coordinate_system", NORMALIZED) != NORMALIZED:
                    raise ValueError(
                        f"unsupported coordinate_system: {row.get('coordinate_system')!r}"
                    )
                ...
                for index, element in enumerate(elements, 1):
                    if not isinstance(element, dict):
                        ...
                        result["errors"].append(
                            f"{row_id} element {index}: element is not an object"
                        )
                        continue
                    ...
                    if bbox is None:
                        result["invalid_bboxes"] += 1
                        result["errors"].append(
                            f"{row_id} element {index}: invalid bbox_norm {element.get('bbox_norm')!r}"
                        )
                        continue
                    ...
                    if focused:
                        label += " [focused]"
                    draw_label(
                        draw, (bbox[0], bbox[1] - 28), label, color, text_color, fnt, width, height
                    )
                    used.append(bbox)
...
def main() -> None:
    ...
    parser.add_argument(
        "--root", type=Path, default=Path("outputs/lora_ablation/collected_predictions_2048")
    )
    ...
    if args.manifest_only:
        for deployment in DEPLOYMENTS:
            ...
            for experiment in EXPERIMENTS:
                summary_path = (
                    root / deployment / experiment / "bbox_visualizations" / "summary.json"
                )
                ...
                manifest[deployment].append(
                    {
                        ...
                        "invalid_bboxes": summary["invalid_bboxes"],
                    }
                )
        ...
        print(
            json.dumps(
                {d: len(manifest[d]) for d in DEPLOYMENTS} | {"manifest": str(manifest_path)},
                ensure_ascii=False,
            )
        )
        return
    if args.baseline:
        ...
        entries[-1]["visualization_dir"] = str(
            (root / "baselines" / experiment / "bbox_visualizations").resolve()
        )
        ...
        collection_entries = [
            item for item in collection_entries if item.get("experiment") != experiment
        ]
        collection_entries.append(
            {
                ...
                "collected": str(
                    Path("outputs/lora_ablation/collected_predictions_2048")
                    / "baselines"
                    / experiment
                ),
                "prediction_file": str(
        ...
        print(
            json.dumps({"baseline": experiment, "manifest": str(manifest_path)}, ensure_ascii=False)
        )
        return
    ...
    for deployment in deployments:
        if not args.experiment:
            ...
            manifest[deployment] = [
                item for item in manifest[deployment] if item.get("experiment") != args.experiment
            ]
        for experiment in experiments:
            ...
            manifest[deployment][-1]["visualization_dir"] = str(
                (root / deployment / experiment / "bbox_visualizations").resolve()
            )
    manifest_path.write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
